...
const ProductDetails = ({ cartItems, addItemToCart }) => {
  ...
  }, [id, error, success, errorReview, isDeleted]);
  ...
  const iswish =
    wishlist && wishlist.product && wishlist.product.status === "wishlisted"
      ? true
      : false;
  ...
  const getWishlist = async (id) => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:8001/api/v1/wishlistshow/${id}`,
        config
      );
      console.log(response.data.product.status);
      console.log(response.data.product._id);

      <FontAwesomeIcon icon={faHeart} color={"#8b4656"} />;
  ...
  const addToWishlist = async (ProductId) => {
    try {
      ...
      const { data } = await axios.post(
        "http://localhost:8001/api/v1/wishlist",
        { product: ProductId },
        config
      );
  ...
  const removeFromWishlist = async (id) => {
    ...
    try {
      ...
      const { data } = await axios.delete(
        `http://localhost:8001/api/v1/deletewish/${id}`,
        config
      );
  ...
  const WishlistHandler = () => {
    ...
    if (
      wishlist &&
      wishlist.product &&
      wishlist.product.status === "wishlisted"
    ) {
      if (wishlist && wishlist.product) {
        deleteOrderHandler(wishlist.product._id);
      } else {
          ...
          <div className="row d-flex justify-content-around">
            ...
            <div className="col-12 col-lg-5 mt-5">
              ...
              >
                <FontAwesomeIcon
                  icon={faHeart}
                  color={iswish ? "#8b4656" : "gray"}
                />

                {/* <span className={iswish ? "greenColor" : "redColor"}>
                  Favorite
                </span> */}
              </button>
